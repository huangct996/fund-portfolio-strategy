...
| mkv | float | 持有股票市值（元） | 单位：元，**不用于权重计算**。公式：持仓数量 × 股价 |
| amount | float | 持仓数量（股） | 单位：股 |
| stk_mkv_ratio | float | 占股票市值比 | **关键字段**，百分比（如5.6表示5.6%）。**真实含义**：占基金净资产的比例。公式：(mkv / 基金净资产) × 100。所有持仓的stk_mkv_ratio总和接近100%。用于计算原策略权重 |
| stk_float_ratio | float | 占流通股本比例 | 百分比。公式：(基金持仓数量 / 股票流通股本) × 100。用于评估持仓集中度 |
...
⚠️ **重要：stk_mkv_ratio的真实含义**

官方文档说明"占股票市值比"容易引起误解，通过实际数据验证，该字段的真实含义是：

```
stk_mkv_ratio = (持有股票市值 / 基金净资产) × 100
```

**验证数据**（512890.SH，20230630报告期）：
- 所有持仓的mkv总和：1,243,114,329.84元
- 所有持仓的stk_mkv_ratio总和：99.91%
- 推算基金净资产：1,244,234,140.57元
...
const weight = h.mkv / totalMkv;  // 错误！会忽略现金等非股票资产

// ✅ 正确做法
const weight = h.stk_mkv_ratio / 100;  // 正确！直接使用官方权重
```

详细验证报告请查看：`docs/FIELD_VERIFICATION_REPORT.md`
